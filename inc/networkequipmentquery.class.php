...
/*
 * @version $Id: networkequipmentquery.class.php 196 2016-04-08 21:00:00Z jul-m $
 -------------------------------------------------------------------------
...
class PluginArchiresNetworkEquipmentQuery extends CommonDBTM
{
    ...
    function Query($ID, $PluginArchiresView, $for)
    {
                ...
                if ($end)
                {
                    foreach ($types as $key => $val)
                    {
                        ...
                        $fieldsnp = "`np`.`id`, `np`.`items_id`, `np`.`logical_number`,
                                 `np`.`instantiation_type` AS instantiation_type, `glpi_ipaddresses`.`name` AS ip,
                        ...
                        {
                            $query .= ", `glpi_networkports_vlans` AS nv";
                        }

                        $query .= ")
                    LEFT JOIN `glpi_networknames` ON (`glpi_networknames`.`itemtype` = 'NetworkPort' AND `np`.`id` = `glpi_networknames`.`items_id`)
                    LEFT JOIN `glpi_ipaddresses` ON (`glpi_ipaddresses`.`itemtype` = 'NetworkName' AND `glpi_networknames`.`id` = `glpi_ipaddresses`.`items_id`)
                    LEFT JOIN `glpi_ipaddresses_ipnetworks` ON `glpi_ipaddresses_ipnetworks`.`ipaddresses_id` = `glpi_ipaddresses`.`id`
                    LEFT JOIN `glpi_ipnetworks` ON `glpi_ipnetworks`.`id` = `glpi_ipaddresses_ipnetworks`.`ipnetworks_id`
                    ...
                    AND `$itemtable`.`is_template` = '0'" .
                        ...
                        if ($result = $DB->query($query))
                        {
                            while ($data = $DB->fetch_array($result))
                            {
                                ...
                                $ports[$data["id"]]["instantiation_type"] = 'NetworkPortEthernet';

                                if ($itemtable == "glpi_networkequipments")
                                {
                                    // Get Network Device Management IP
                                    $query = "SELECT `glpi_ipaddresses`.`name` AS ip
                                        FROM `glpi_networkports`
                                        INNER JOIN `glpi_networknames` ON (`glpi_networkports`.`id` = `glpi_networknames`. `items_id`)
                                        LEFT JOIN `glpi_ipaddresses` ON (`glpi_networknames`. `id` = `glpi_ipaddresses`.`items_id`)
                                        WHERE `glpi_networkports`.items_id = " . $data["items_id"] . "
                                        AND `glpi_networkports`.`instantiation_type` = 'NetworkPortAggregate'";
                                    $result_ip = $DB->query($query);
                                    $data_ip = $DB->fetch_array($result_ip);
                                    $devices[$val][$data["items_id"]]["ip"] = $data_ip['ip'];

                                    if (!empty($devices[$val][$data["items_id"]]["ip"]))
                                    {
                                        if (strpos($devices[$val][$data["items_id"]]["ip"], $data_ip['ip']))
                                        {
        ...
        //The networking
        $query = "SELECT `n`.`id` AS `idn`, `np`.`id`, `np`.`items_id`, `np`.`logical_number`, np.instantiation_type,
                    `glpi_ipaddresses`.`name` AS ip,
                    `np`.`name` AS `namep`,
                    `ipn`.`netmask`, `n`.`name`,
                    `n`.`networkequipmenttypes_id` AS `type`, `n`.`users_id`, `n`.`groups_id`,
                    `n`.`contact`, `n`.`states_id`, `n`.`entities_id`,`n`.`locations_id`,
                    `glpi_ipaddresses`.`name` AS nip
                FROM (`glpi_networkports` `np`, `glpi_networkequipments` `n`, `glpi_ipnetworks` AS ipn)
